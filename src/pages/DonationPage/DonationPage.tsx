...
const generateAndSaveUserCode = async (user: any): Promise<string> => {
	...
	console.log(
		`Generated code ${result} for user ${user?.id}. (This should be a backend call)`
	)
	try {
		...
		if (!response.ok) {
			...
			throw new Error(
				errorData.error?.message || 'Ошибка при сохранении кода на сервере'
			)
		}

		console.log(
			`Сгенерирован и сохранен новый код ${result} для пользователя ${user.id}`
		)
...
const DonationPage: React.FC = () => {
	...
	const handleCheckPayment = async () => {
		...
		try {
			...
			const res: CheckPaymentResponse & {
				created?: boolean
				duplicated?: boolean
			} = await checkPaymentByCode(user?.unique_code ?? '', {
				timeoutMs: 12000,
			})
			...
			switch (true) {
				...
				case res.found === true &&
					...
					setMsg(
						res.reason ||
							'Платёж найден, перезагрузите страницу или не удалось зачислить оплату.'
					)
					break
				...
				default:
					setStatus('notfound')
					setMsg(
						res.reason || 'Оплата пока не найдена. Попробуйте через минуту.'
					)
					break
			}
		} catch (e: any) {
			...
			<div className='donation-panel'>
				...
				<div className='donation-content'>
					...
					<p className='instructions important'>
						ВАЖНО! Чтобы CR были зачислены, обязательно укажите в комментарии к
						платежу ваш уникальный код:
					</p>
				...
				<button
					className='back-button-DonationPage'

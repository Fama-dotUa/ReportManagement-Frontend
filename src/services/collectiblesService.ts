...
const collectiblesService = {
    ...
    performUpgrade(itemId: number): { success: boolean, message: string } {
        ...
        let newInventory = this.state.inventory.map(invItem => {
            if (invItem.itemId === itemId) return { ...invItem, quantity: invItem.quantity - upgradeInfo.itemsRequired };
            if (invItem.itemId === DUST_ITEM_ID) return { ...invItem, quantity: invItem.quantity - upgradeInfo.dustCost };
            if (invItem.itemId === newItem.id) {
                newItemAdded = true;
        ...
        if (!newItemAdded) {
            newInventory = [...newInventory, { itemId: newItem.id, quantity: 1 }];
        }
    ...
    buyPack(collectionName: string): { success: boolean, message: string, itemsReceived?: string[] } {
        ...
        // ИСПРАВЛЕНИЕ: Начинаем с копии инвентаря и обновляем ее итеративно
